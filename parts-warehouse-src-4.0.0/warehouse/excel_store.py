...
class ExcelStore:
    ...
    def recent_items(self, limit: int = 5) -> list:
        """主页近期库存: 从当前真实 Excel 行生成轻量预览记录。"""
        items = []
        for subcat, rows in self._cached_stats_rows().items():
            for row in rows:
                padded = list(row) + [""] * 5
                items.append({
                    "subcat": subcat,
                    "name": padded[0] or "(未命名)",
                    "brand": padded[1] or "",
                    "qty": padded[3] or 0,
                    "location": padded[4] or "",
                })
