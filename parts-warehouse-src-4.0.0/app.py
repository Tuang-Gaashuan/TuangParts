...
def settings_post():
    ...
    if "bg_fill" in data:
        ...
        if v in ("cover", "contain", "stretch", "repeat", "original"):
            patch["bg_fill"] = v
    for k in ("bg_brightness", "bg_opacity", "card_opacity", "font_size", "dark_strength"):
        if k in data:
            try:
                limits = {"font_size": (80, 130), "dark_strength": (0, 100)}
                lo, hi = limits.get(k, (0, 200))
                patch[k] = max(lo, min(hi, int(data[k])))
            except (TypeError, ValueError):
                pass
    if "font_family" in data:
        v = str(data.get("font_family") or "system").strip()
        if v in ("system", "microsoft", "segoe", "mono"):
            patch["font_family"] = v
    if "ai" in data:
        new_ai = {
...
def global_search():
    ...
    store = get_store()
    items = []
    for subcat, rows in store._cached_stats_rows().items():
        owner = primary_owner(subcat)
        owner_name = CATEGORIES[owner][0] if owner in CATEGORIES else ""
        for index, row in enumerate(rows):
            values = [str(v or "") for v in row]
            if q not in " ".join(values).lower() and q not in subcat.lower() and q not in owner_name.lower():
                continue
            padded = values + [""] * 9
            items.append({"subcat": subcat, "category": owner_name, "name": padded[0], "brand": padded[1], "package": padded[2], "qty": padded[3], "location": padded[4], "spec": padded[6], "row": index})
    return jsonify({"items": items[:100], "total": len(items)})
...
def dashboard():
    ...
    return jsonify({"stats": stats, "logs": logs, "recent_items": store.recent_items()})
